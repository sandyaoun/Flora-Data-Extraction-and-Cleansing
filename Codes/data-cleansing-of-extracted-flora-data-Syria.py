	...
	for extract_flora_data_from_book in extracted_flora_data_from_books_list:
		...
		with open(extract_flora_data_from_book, newline='') as csvfile:
			...
			for flora_data_row in extracted_flora_data:
				...
				if(flora_data_row['SyrianLocations'] != '0'):
					useful_row_info = []
					useful_row_info.append('Syria')				
					useful_row_info.append(flora_data_row['SyrianLocations'])
					first_destination_file.write(",".join(useful_row_info) + "\n")
		...
		for info_line_nbr,info_line in enumerate(info_lines_list):
			if info_line.startswith('Syria'):
				if info_line == 'Syria,S.' or re.match('^.*\(.*\)\.$',info_line):
				...
				if lebanon_locations_info != '':
					cleaned_lebanon_locations_info = re.sub('^Syria,S\.\s|^Syria,S\.','Syria,',lebanon_locations_info)
		...
		for info_line_nbr,info_line in enumerate(info_lines_list):	
			if (not info_line.startswith('Syria')):
				if info_line_nbr != len(info_lines_list) - 1:
					if info_lines_list[info_line_nbr+1].startswith('Syria'):
						useful_row_info = []
						useful_row_info.append(info_line)
		...
		for info_line_nbr,info_line in enumerate(info_lines_list):		
			if (not info_line.startswith('Syria')):
				for line_nbr in range(info_line_nbr+1,len(info_lines_list)):
					...
					if info_line_internal.startswith('Syria'):
						useful_row_info = []
		...
		for line_nbr in range(len(info_lines_list)):
			genus,epithet,syria,locations = info_lines_list[line_nbr].split(",",3)
		...
		for line_nbr in range(len(info_lines_list)):
			...
			if locations != '':
				...
				for sub_location in sub_locations:
					if(len(sub_location)>4 and len(sub_location)<40):
						useful_row_info = []
						useful_row_info.append(genus.strip())
						useful_row_info.append(epithet.strip())
						useful_row_info.append(syria.strip())
		...
		end_result_file_name = 'cleansed-extracted-flora-data-Syria-from-book-0' + str(extracted_flora_data_book_number) + '.txt'
		os.rename(sixth_destination_file_name,end_result_file_name)
		
		
		current_path = os.getcwd()
		destination_file_path = current_path + '/cleansed-extracted-flora-data-Syria-from-book-0' + str(extracted_flora_data_book_number) + '.txt'
		destination_file_csv_path = current_path + '/cleansed-extracted-flora-data-Syria-from-book-0' + str(extracted_flora_data_book_number) + '.csv'
